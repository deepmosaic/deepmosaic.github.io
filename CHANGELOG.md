...
> **チケットは親リポジトリ `../CHANGELOG.md` に記載する (T-175 で集約済み)。** 本ファイルは履歴の参照用に残しているだけで、以後は追記しない。

## 2026-09-05 - Fix: docs のスクショからメールアドレスを除去 + 目次のスクロールバー (TICKET-SITE-49 / 50)

ユーザー指摘 2 件。(1) お問い合わせダイアログやメニューバーのプロフィール表示に、撮影機の
ログインアカウントのメールアドレスがそのまま写っていた。(2) 縦スクロールする目次に OS 既定の
スクロールバー (明るいトラック・角ばったつまみ) が出て、ダークの側柱で浮いていた。

- [x] TICKET-SITE-49: desktop の撮影ハーネスにメールアドレスの伏せ字 (`user@example.com`、T-168) を
  入れて 36 枚すべてを撮り直し、WebP 化して差し替え
- [x] TICKET-SITE-50: `nav.docs-toc` に細い丸型のスクロールバー (`scrollbar-width: thin` +
  `::-webkit-scrollbar` 6px、トラック透明、hover で少し明るく) を追加
...
- [x] TICKET-SITE-44: 導入〜検出の執筆 — 画面の構成 (`#layout`) / テーマ (`#theme`) / ログイン手順・
...
- [x] TICKET-SITE-45: 編集の執筆 — 再生コントロール・検出枠の操作・右クリックメニュー・モザイク除外 /
...
- [x] TICKET-SITE-46: 書き出し〜運用の執筆 — エンコードの全選択肢と前回設定の再利用 / タイムコードの
