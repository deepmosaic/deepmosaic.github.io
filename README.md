...
| `scripts/optimize-images.mjs` | PNG → WebP 変換 (ffmpeg)。既定の対象は `assets/img/screenshots/` 直下だけ、`<dir>` で明示も可。引数なしは一覧表示のみ、`--apply` で変換して元 PNG を削除。原稿から参照中の PNG は `[KEEP]` で残す (T-522、判定は `src/lib/image-optimize.js`) |
...
- **画像追加時の運用**（WebP 必須・`node scripts/optimize-images.mjs --apply` 等）は `CLAUDE.md` の Conventions を参照。既定の対象は `assets/img/screenshots/` だけで、`assets/img/` 直下のロゴ (JSON-LD が参照する `logo-font.png` など) は巻き込まない。
